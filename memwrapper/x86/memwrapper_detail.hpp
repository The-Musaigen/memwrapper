...
namespace memwrapper
{
	namespace detail
	{
		...
		inline uint32_t get_relative_address(const memory_pointer& to, const memory_pointer& from, size_t oplen = 5)
		{
			return (to.get_int() - from.get_int() - oplen);
		}

		inline uint32_t restore_absolute_address(const memory_pointer& imm, const memory_pointer& from, size_t oplen = 5)
		{
	...
	{
		return detail::function_caller<Convention>::template call<Ret, Args...>(fn, args...);
	}

	template<typename Ret, typename... Args>
	inline Ret call_cdecl(const memory_pointer& fn, Args... args)
	{
		return call_function<Ret, detail::CallingConvention::Cdecl, Args...>(fn, args...);
	}

	template<typename Ret, typename... Args>
	inline Ret call_winapi(const memory_pointer& fn, Args... args)
	{
		return call_function<Ret, detail::CallingConvention::Stdcall, Args...>(fn, args...);
	}

	template<typename Ret, typename... Args>
	inline Ret call_method(const memory_pointer& fn, Args... args)
	{
		return call_function<Ret, detail::CallingConvention::Thiscall, Args...>(fn, args...);
	}

	template<typename Ret, typename... Args>
	inline Ret call_fast(const memory_pointer& fn, Args... args)
	{
		return call_function<Ret, detail::CallingConvention::Fastcall, Args...>(fn, args...);
	}
} // !namespace memwrapper
