...
namespace memwrapper
{
	...
	namespace detail
	{
		static uint32_t convert_memory_protection_constant(MemoryProt prot)
		{
			switch (prot)
			{
			case MemoryProt::NoAccess:
				return PAGE_NOACCESS;
			case MemoryProt::ReadOnly:
				return PAGE_READONLY;
			case MemoryProt::ReadWrite:
				return PAGE_READWRITE;
			case MemoryProt::WriteCopy:
				return PAGE_WRITECOPY;
			case MemoryProt::Execute:
				return PAGE_EXECUTE;
			case MemoryProt::ExecuteRead:
				return PAGE_EXECUTE_READ;
			case MemoryProt::ExecuteReadWrite:
				return PAGE_EXECUTE_READWRITE;
			case MemoryProt::ExecuteWriteCopy:
	...
	protected:
		...
		bool	m_result;
	public:
		scoped_unprotect(const memory_pointer& pointer, size_t size, MemoryProt prot = MemoryProt::ExecuteReadWrite) :
			m_pointer(pointer),
			m_size(size)
		{
			auto new_protect = detail::convert_memory_protection_constant(prot);

			m_result = (VirtualProtect(m_pointer, m_size, new_protect, &m_protection) != 0);
		}
	...
	inline void copy_memory(const memory_pointer& dst, const memory_pointer& src, size_t size)
	{
		scoped_unprotect unprotect(dst, size);
	...
	inline int compare_memory(const memory_pointer& buff1, const memory_pointer& buff2, size_t size)
	{
	...
	{
		memory_pointer handle = reinterpret_cast<uint32_t>(GetModuleHandle(mod.data()));
	...
	public:
		...
		scoped_patch_unit(std::string_view mod, const memory_pointer& offset, const byte_vector& replacement, const byte_vector& original) :
		...
		scoped_patch_unit(std::string_view mod, const memory_pointer& offset, const byte_vector& replacement) :
			m_replacement(replacement)
		{
			auto handle = reinterpret_cast<uint32_t>(GetModuleHandle(mod.data()));
		...
		scoped_patch_unit(const memory_pointer& address, const byte_vector& replacement, const byte_vector& original) :
			m_address(address),
			m_replacement(replacement),
			m_original(original)
		{}

		scoped_patch_unit(const memory_pointer& address, const byte_vector& replacement) :
			m_address(address),
			m_replacement(replacement)
		{
			m_original.resize(replacement.size());
